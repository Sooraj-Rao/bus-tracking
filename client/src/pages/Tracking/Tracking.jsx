...
const Tracking = () => {
  ...
  const fetchBuses = async () => {
    try {
      ...
      setBuses(res.data)
      showInfo("Data Updated", "Bus locations refreshed successfully")
    } catch (error) {
    ...
    <div className="tracking-page">
      <div className="container">
        <div className="tracking-header">
          ...
          <p className="tracking-subtistle">Track all buses in real-time and get live location updates</p>
        </div>
        ...
        <div className="bus-list">
          <h3 className="bus-list-title">Available Buses ({filteredBuses.length})</h3>
              ...
              {filteredBuses.map((bus) => (
                <div key={bus._id} className="bus-card">
                  ...
                  <div className="bus-info">
                    ...
                    <div>
                      <strong>Location:</strong> {bus.currentLocation.lat.toFixed(4)},{" "}
                      {bus.currentLocation.lng.toFixed(4)}
                    </div>
                    <div>
                      <strong>Last Updated:</strong> {new Date(bus.currentLocation.timestamp).toLocaleString()}
                    </div>
                  </div>

                  <button className="track-button" onClick={() => handleTrackBus(bus._id)}>
                    📍 Track This Bus
                  </button>
                </div>
              ))}
